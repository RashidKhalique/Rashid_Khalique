...
const Project = () => {
    ...
    const nextproject = ()=>{
        setcurrentstate((prevIndex)=>(prevIndex + 1) % projectsData.length+4)
    }
    const Prevproject = ()=>{
        setcurrentstate((prevIndex)=> prevIndex === 0 ? projectsData.length - 2 : prevIndex - 1)
    }
            ...
            <div className='overflow-hidden'>
                <div className='flex gap-8 transition-transform duration-500 ease-in-out ' style={{transform : `translateX(-${(currentstate * 100)/cardshow}%)`}}>
                    {projectsData.map((project, index) => (
                        <div key={index} className='relative flex-shrink-0 w-full sm:w-1/4 ' style={{backgroundImage: `url(${project.image})`, backgroundSize: 'cover', backgroundPosition: 'start', height: '300px'}} onClick={()=>window.open(project.link)}>
